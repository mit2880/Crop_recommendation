...
def main():
    ...
    n = st.number_input('Nitrogen', key="Nitrogen", format="%i", step=1)
    n = (n-0)/120
    p = st.number_input('Phosphorus', key="phosphrous", format="%i", step=1)
    p = (p-5)/140
    k = st.number_input('Potassium', key="potassium", format="%i", step=1)
    k = (k-5)/200
    Temperature = st.number_input('Temperature', key="temp", format="%f", step=0.5)
    Temperature = (Temperature-8.825674745)/34.84981831
    Humidity = st.number_input('Humidity', key="humidity", format="%f", step=0.5)
    Humidity = (Humidity-45.02236377)/54.95951224
    ph = st.number_input('pH', key="ph", format="%f", step=0.5)
    ph = (ph-4.507523551)/3.488325426
    Rainfall = st.number_input('Rainfall', key="rainfall", format="%f", step=0.5)
